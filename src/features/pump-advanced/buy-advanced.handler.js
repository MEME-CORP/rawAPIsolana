...
import { schema } from './buy-advanced.schema.js';
...
/**
 * POST /pump/advanced-buy
 * Build unsigned Pump.fun buy tx, sign with provided private key, send, confirm, and return post SOL+SPL balances.
 ...
 * @param {Response} res
 */
export async function buyAdvancedHandler(req, res) {
  const parsed = schema.parse(req.body);
  const priorityFeeSol = parsed.priorityFeeSol ?? DEFAULT_PRIORITY_FEE_SOL;

  // Decode signer and validate it matches buyerPublicKey
  let signer, buyerPk, mintPk;
  try {
    buyerPk = new PublicKey(parsed.buyerPublicKey);
    mintPk = new PublicKey(parsed.mintAddress);
  } catch {
    throw new ApiError('INVALID_INPUT', 'buyerPublicKey or mintAddress is not valid', 400);
  }
  try {
    ...
    if (!buyerPk.equals(signer.publicKey)) {
      throw new ApiError('INVALID_INPUT', 'privateKey does not match buyerPublicKey (fee payer)', 400);
    }
  } catch (e) {
  ...
  // 1) Request unsigned buy tx from Pump Portal
  const body = {
    publicKey: buyerPk.toBase58(),
    action: 'buy',
  ...
  const lamports = await connection.getBalance(buyerPk, 'confirmed');
  const solPost = {
    publicKey: buyerPk.toBase58(),
  ...
  let splPost = { walletPublicKey: buyerPk.toBase58(), mintAddress: mintPk.toBase58(), uiAmount: 0, rawAmount: '0' };
  try {
    const respTok = await connection.getParsedTokenAccountsByOwner(buyerPk, { mint: mintPk }, 'confirmed');
    if (respTok?.value?.length > 0) {
      ...
      splPost = { walletPublicKey: buyerPk.toBase58(), mintAddress: mintPk.toBase58(), uiAmount, rawAmount: amountStr };
    }
  } catch (_) {}
